...
public final class ActionBarHudElement {
    ...
    /*
     * Same as above, but for when the health/hunger/XP rows are showing
     * WITHOUT the armor row on top of them (VanillaHud#hasArmor is false,
     * i.e. player.getArmor() == 0) - one icon row shorter than the full
     * EXTRA_HUD_LIFT, so we don't lift as high.
     */
    private static final int EXTRA_HUD_LIFT_NO_ARMOR = 12;
    ...
    private static void drawBar(DrawContext context, MinecraftClient client) {
        ...
        int extraHudLift = 0;
        if (VanillaHud.hasExtraHud(client)) {
            extraHudLift = VanillaHud.hasArmor(client) ? EXTRA_HUD_LIFT : EXTRA_HUD_LIFT_NO_ARMOR;
        }
